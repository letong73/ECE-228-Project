...
def accuracy_curve_multi(snrs, acc_dict):
    """
    画多条模型在不同 SNR 上的准确率曲线。

    参数:
    - snrs: list of SNRs (例如 [-20, -10, 0, 10, 20])
    - acc_dict: dict，键是模型名，值是一个 dict，其键是 SNR，值是 accuracy。
      例如：
        {
          "CNN2": {-20: 0.1, -10: 0.3, 0: 0.6, 10: 0.8, 20: 0.9},
          "LSTM": {-20: 0.15, -10: 0.35, 0: 0.65, 10: 0.85, 20: 0.95}
        }
    """
    import matplotlib.pyplot as plt

    for model_name, acc in acc_dict.items():
        acc_values = [acc[snr] for snr in snrs]
        plt.plot(snrs, acc_values, label=model_name)
